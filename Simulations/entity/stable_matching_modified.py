...
#STEP 1 !!! ===> Check if deployment is possible in active servers
    ...
    def check_for_inactive_servers():
        ...
        vnfs_assignment = list(set(vnfs_to_be_deployed_in_inactiveservers.copy()) - set(vnfs_to_be_deployed_in_inactivefacility.copy()))
        for server in inactiveserverlist:
            if server.id not in failing_server_id:
                server_preferences[server.id] = [vnf for vnf in vnfs_assignment]
                # Servers prefer VNFs that can fit into their available resources while maintaining reliability
                # for vnf in vnfs_assignment:
                #     if server.available_resources >= vnf.resources:
                #         server_preferences[server.id].append(vnf)
    ...
    def check_for_inactive_facilities():
        ...
        vnfs_assignment = vnfs_to_be_deployed_in_inactivefacility.copy()
        for server in inactive_facility_serverlist:
            if server.id not in failing_server_id:
                server_preferences[server.id] = [vnf for vnf in vnfs_assignment]
                # Servers prefer VNFs that can fit into their available resources while maintaining reliability
                # for vnf in vnfs_assignment:
